...
class DownBlock(nn.Module):
    ...
    def __init__(self, in_c: int, out_c: int) -> None:
        ...
        # 4x4 kernel, stride 2, padding 1 halves spatial size
        self.pool = nn.Conv2d(out_c, out_c, kernel_size=4, stride=2, padding=1)
...
class UpBlock(nn.Module):
    ...
    def __init__(self, in_channels_prev_decoder: int,
                 skip_channels: int,
                 out_channels: int) -> None:
        super().__init__()
        self.up = nn.ConvTranspose2d(
            in_channels_prev_decoder,
            skip_channels,
            kernel_size=4,
            stride=2,
            padding=1,
...
class UNetAutoencoder(nn.Module):
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, Tuple[torch.Tensor, ...]]:
    ...
    def decode(self, z: torch.Tensor,
               skips: Sequence[torch.Tensor]) -> torch.Tensor:
        s1, s2, s3 = skips
...
def positional_time_embedding(timesteps: torch.Tensor,
                              dim: int = 128) -> torch.Tensor:
    """
    Sinusoidal time embedding as in many diffusion models.
    timesteps: LongTensor of shape [B]
    dim: embedding dimension
    """
    device = timesteps.device
    half = dim // 2
    freqs = torch.exp(
        -math.log(1e4) * torch.arange(0, half, device=device) / max(half, 1)
    )
...
class EMA:
    """Exponential Moving Average over model parameters/buffers."""
